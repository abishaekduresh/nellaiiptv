...
const ClientSessionsMap = dynamic(
  () => import("@/components/ClientSessionsMap"),
  { ssr: false }
);
...
> = {
  online: {
    dot: "bg-green-400 shadow shadow-green-400/50 animate-pulse",
    label: "Online",
...
export default function ClassicHome({
  ...
  const [rightPanel, setRightPanel] = useState<"channels" | "streams">(
    "channels"
  );
  ...
  const fetchMyStreams = useCallback(
    async (force = false): Promise<MyStream[] | null> => {
      try {
  ...
  const handleStreamRestart = async (streamUuid: string) => {
    ...
    try {
      await api.post(`/customers/streams/${streamUuid}/toggle`, {
        enable: false,
      });
    ...
    const fetchSettings = async () => {
      try {
        ...
        if (response.data.status) {
          // Logo Logic
          const logo =
            response.data.data.logo_url || response.data.data.logo_path;
          if (logo) {
            setLogoUrl(resolveImageUrl(logo));
          }

          // Top Trending Logic (Classic = tv)
          const platforms = response.data.data.top_trending_platforms || [
            "web",
            "android",
            "ios",
            "tv",
  ...
  // Filtering State
  const [groupBy, setGroupBy] = useState<"all" | "language" | "category">(
    "all"
  );
  ...
  useEffect(() => {
    ...
    fetch(`${base}/visual-ads/active`, {
      ...
      .then((data: any) => {
        ...
        if (!ad) return;
        if (
          ad.max_impressions_per_session > 0 &&
          getSessionImpressions(ad.uuid) >= ad.max_impressions_per_session
        )
          return;
  ...
  const handleChannelClick = useCallback(
    async (channel: Channel, source: string = "main") => {
      // Optimistic update
      ...
      try {
        ...
        if (res.data.status && res.data.data) {
          ...
          setSelectedChannel((prev) =>
            prev?.uuid === fresh.uuid ? fresh : prev
          );
        }
      } catch (e) {
    ...
    if (channels.length > 0 && !selectedChannel) {
      ...
      api.get(`/channels/${first.uuid}`).then((res) => {
        if (res.data.status && res.data.data) {
          ...
          setSelectedChannel((prev) =>
            prev?.uuid === first.uuid || !prev ? res.data.data : prev
          );
        }
      });
    }
  ...
  */
  ...
  const rating =
    Number(selectedChannel.ratings_avg_rating || 0) ||
    Number(selectedChannel.average_rating) ||
    0;
  ...
  const getPriority = useCallback(
    (key: string, type: "language" | "category") => {
  ...
  const effectiveActiveGroup =
    activeGroup || (groupBy === "all" ? "All Channels" : groupKeys[0]);
  const displayChannels =
    groupBy === "all" ? channels : groups[effectiveActiveGroup] || [];
  ...
  return (
    <div
      ref={topRef}
      className="w-full px-0 py-0 lg:px-4 lg:py-2 h-auto lg:h-[calc(100vh)] overflow-y-auto lg:overflow-hidden"
    >
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <div className="flex items-center justify-between gap-3">
                ...
                <div className="bg-slate-950/80 rounded border border-slate-800 px-2 py-1 flex items-center gap-3 shrink-0">
                  <div className="flex flex-col items-center">
                    ...
                    <span className="text-[8px] text-slate-500 font-medium uppercase tracking-wider scale-90">
                      Views
                    </span>
                  </div>
              ...
              <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-slate-400 text-[10px] lg:text-xs pl-0.5">
                ...
                <div className="flex items-center gap-1 border-l border-slate-700/50 pl-3">
                  <span className="text-slate-500 text-[9px] mr-1 hidden sm:inline">
                    Rate:
                  </span>
                  <div className="flex">
                    {[1, 2, 3, 4, 5].map((star) => (
                      <RatingStar
                        ...
                        onRate={(val) => {
                          api
                            .post(`/channels/${selectedChannel.uuid}/rate`, {
                              rating: val,
                            })
                            .then((res) => {
                              if (res.data.status && res.data.data) {
                                ...
                                setSelectedChannel((prev) =>
                                  prev ? { ...prev, user_rating: val } : null
                                );
                              }
                            })
                            .catch((err) =>
              ...
              <div className="flex items-center gap-2 lg:gap-3">
                ...
                <Link href="/">
                  <img
                    ...
                    alt="Logo"
                    className="w-10 h-10 lg:w-12 lg:h-12 rounded-lg object-contain bg-black cursor-pointer hover:scale-105 transition-transform"
                  />
                </Link>
                <div>
                  <h2 className="font-bold text-white text-sm lg:text-base leading-tight">
                    Nellai IPTV
                  </h2>
            ...
            {user && myStreams.length > 0 && (
              <div className="flex gap-1 bg-slate-950 rounded-lg p-1 border border-slate-800">
                ...
                >
                  ...
                  <span className="bg-cyan-500 text-slate-900 text-[9px] font-bold rounded-full w-3.5 h-3.5 flex items-center justify-center">
                    {myStreams.length}
                  </span>
                </button>
              </div>
            ...
            {rightPanel === "channels" && (
              <>
                <div className="flex gap-2">
                  <button
                    {...groupFocus}
                    className={`flex-1 flex items-center justify-between bg-slate-950 text-white text-xs p-2 lg:p-2.5 rounded border transition-all ${
                      isGroupFocused
                        ? "border-primary ring-2 ring-primary/50 bg-slate-900"
                        : "border-slate-800"
                    }`}
                  >
                    <div className="flex items-center gap-2">
            ...
            <div className="overflow-y-auto flex-1 scrollbar-hide flex flex-col">
              ...
              <div className="flex items-center justify-between px-3 py-2 border-b border-slate-800/60 shrink-0">
                <span className="text-[10px] text-slate-500 uppercase tracking-wider font-semibold">
                  {myStreams.length} stream{myStreams.length !== 1 ? "s" : ""}{" "}
                  assigned
                </span>
                <button
              ...
              <div className="p-2 grid grid-cols-2 sm:grid-cols-3 gap-2">
                {myStreams.map((s) => {
                  const health =
                    HEALTH_CFG[s.health_status ?? "offline"] ??
                    HEALTH_CFG.offline;
                    ...
                    >
                      ...
                      >
                        <RotateCcw
                          size={10}
                          className={
                            restarting === s.uuid ? "animate-spin" : ""
                          }
                        />
                      </button>
                      ...
                      <div className="flex items-start gap-2 pr-5">
                        <div
                          className={`w-2.5 h-2.5 rounded-full shrink-0 mt-0.5 ${health.dot}`}
                        />
                        <p className="text-white text-sm font-semibold leading-tight line-clamp-2 break-all">
                          {s.stream_name}
                        </p>
                      </div>
                      ...
                      <div className="flex flex-wrap gap-1">
                        <span
                          className={`inline-flex items-center px-2 py-0.5 rounded border text-[11px] font-semibold ${health.badge}`}
                        >
                          {health.label}
                        </span>
                        {streamSt && (
                          <span
                            className={`inline-flex items-center px-2 py-0.5 rounded border text-[11px] font-semibold ${streamSt.badge}`}
                          >
                            {streamSt.label}
                          </span>
                        )}
                      ...
                      <div className="flex items-center justify-between text-[11px] text-slate-500 mt-auto">
                        ...
                        {s.uptime ? (
                          <span>⏱ {fmtUptimeClassic(s.uptime)}</span>
                        ) : null}
                      </div>
                    </div>
                  );
            ...
            <div className="overflow-visible lg:overflow-y-auto p-2 lg:p-3 flex-1 scrollbar-hide space-y-4 lg:space-y-6">
              ...
              {showTopTrending &&
                ...
                groupBy === "all" && (
                  <div className="mb-2 lg:mb-4">
                    <h3 className="text-[10px] lg:text-xs font-bold text-primary mb-1.5 lg:mb-2 uppercase tracking-wider pl-1 border-l-2 border-primary">
                      Top Trending
                    </h3>
                    <div className="grid grid-cols-2 lg:grid-cols-4 gap-2">
                      {" "}
                      {/* Changed to 4 columns desktop, 2 mobile */}
                      {topTrending
                        .map(
                          (t) => channels.find((c) => c.uuid === t.uuid) || t
                        )
                        .sort(
                          (a, b) =>
                            (b.viewers_count || 0) - (a.viewers_count || 0)
                        )
                        .slice(0, 8)
                        .map((enrichedChannel, i) => (
                          <ChannelListItem
                            ...
                            index={i}
                            isActive={
                              selectedChannel.uuid === enrichedChannel.uuid &&
                              selectedSource === "trending"
                            }
                            onSelect={() =>
                              handleChannelClick(enrichedChannel, "trending")
                            }
                            compact={true}
                          />
                        ))}
              ...
              <div>
                <h3 className="text-[10px] lg:text-xs font-bold text-slate-400 mb-1.5 lg:mb-2 uppercase tracking-wider pl-1">
                  {groupBy === "all"
                    ? "All Channels"
                    : effectiveActiveGroup || "Select Group"}{" "}
                  <span className="text-slate-600">
                    ({displayChannels.length})
                  </span>
                </h3>
                <div className="grid grid-cols-3 sm:grid-cols-3 lg:grid-cols-4 gap-2">
                  {displayChannels.reduce(
                    (acc: React.ReactNode[], channel, index) => {
                      acc.push(
                        <ChannelListItem
                          key={channel.uuid}
                          channel={channel}
                          index={index}
                          isActive={
                            selectedChannel.uuid === channel.uuid &&
                            selectedSource === "main"
                          }
...
function StreamDetailModal({
  ...
  const clientPct =
    s.max_sessions && s.max_sessions > 0
      ? Math.min(
          100,
          Math.round(((s.online_clients ?? 0) / s.max_sessions) * 100)
        )
      : 0;
    ...
    <div className="fixed inset-0 z-[9999] bg-slate-950 flex flex-col overflow-hidden">
      ...
      <div className="flex items-center gap-3 px-4 py-3 border-b border-slate-800 bg-slate-900 shrink-0">
        ...
        <div className="flex items-center gap-2 min-w-0 flex-1">
          ...
          <span
            className={`hidden sm:inline-flex items-center px-2 py-0.5 rounded border text-xs font-semibold shrink-0 ${health.badge}`}
          >
            {health.label}
          </span>
          {streamSt && (
            <span
              className={`hidden sm:inline-flex items-center px-2 py-0.5 rounded border text-xs font-semibold shrink-0 ${streamSt.badge}`}
            >
              {streamSt.label}
            </span>
          )}
        ...
        <button
          onClick={onSync}
          disabled={syncing || syncCooldown > 0}
          className="flex items-center gap-1.5 px-3 py-2 rounded-lg bg-cyan-500/10 border border-cyan-500/20 text-cyan-400 hover:bg-cyan-500/20 transition-colors disabled:opacity-40 text font-semibold shrink-0"
        >
        ...
        >
          <RotateCcw
            size={14}
            className={restarting === s.uuid ? "animate-spin" : ""}
          />
          <span className="hidden sm:inline">
            {restarting === s.uuid ? "Restarting…" : "Restart"}
          </span>
        </button>
      </div>
      ...
      <div className="flex-1 overflow-y-auto scrollbar-hide">
        {/* Map — shows when any client has exact lat/lng OR a known country code */}
        {s.clients.some(
          (c) =>
            (c.latitude != null && c.longitude != null) ||
            (c.country_code != null &&
              c.country_code.toUpperCase() !== "NONE") ||
        ...
        <div className="p-4 space-y-5">
          ...
          <div className="flex flex-wrap gap-1.5 sm:hidden">
            <span
              className={`inline-flex items-center px-2 py-0.5 rounded border text-xs font-semibold ${health.badge}`}
            >
              {health.label}
            </span>
            {streamSt && (
              <span
                className={`inline-flex items-center px-2 py-0.5 rounded border text-xs font-semibold ${streamSt.badge}`}
              >
                {streamSt.label}
              </span>
            )}
          ...
          {/* Two-column info grid on md+, stacked on mobile */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-xl p-3 space-y-1">
              ...
              <InfoRow
                label="Sessions"
                value={`${s.online_clients ?? 0} / ${s.max_sessions ?? "∞"}`}
              />
              {s.max_sessions && s.max_sessions > 0 && (
                <div className="pt-1">
                ...
                <div className="bg-slate-900 border border-slate-800 rounded-xl p-3 space-y-1">
                  ...
                  <div className="mt-1 space-y-1">
                    {s.video_codec && (
                      <InfoRow
                        label="Codec"
                        value={s.video_codec.toUpperCase()}
                      />
                    )}
                    {s.video_width && s.video_height && (
                      <InfoRow
                        label="Resolution"
                        value={`${s.video_width}×${s.video_height}`}
                      />
                    )}
                ...
                <div className="bg-slate-900 border border-slate-800 rounded-xl p-3 space-y-1">
                  ...
                  <div className="mt-1 space-y-1">
                    {s.audio_codec && (
                      <InfoRow
                        label="Codec"
                        value={s.audio_codec.toUpperCase()}
                      />
                    )}
                    {s.audio_channels && (
                      <InfoRow
                    ...
                    {s.audio_bitrate && (
                      <InfoRow label="Bitrate" value={fmtBw(s.audio_bitrate)} />
                    )}
                    {s.audio_sample_rate && (
                      <InfoRow
                        label="Sample Rate"
                        value={`${(s.audio_sample_rate / 1000).toFixed(1)} kHz`}
                      />
                    )}
                  </div>
                </div>
                        ...
                        return (
                          ...
                          >
                            ...
                            <td className="px-5 py-3 whitespace-nowrap">
                              ...
                              {c.ip_type && (
                                <span className="mt-0.5 inline-block px-1.5 py-px rounded text-xs font-mono bg-slate-800 text-slate-500 uppercase">
                                  {c.ip_type}
                                </span>
                              )}
                            </td>
                            ...
                            <td className="px-3 py-3">
                              {c.protocol ? (
                                <span className="px-1.5 py-0.5 rounded text-xs font-mono bg-slate-800 text-slate-300 uppercase">
                                  {c.protocol}
                                </span>
                              ) : (
                              ...
                              {location ? (
                                <div>
                                  ...
                                  <div className="text-slate-600 text-xs mt-0.5 flex items-center gap-1.5">
                                    ...
                                    {c.country_code && (
                                      <span className="px-1 py-px rounded bg-slate-800 font-mono uppercase">
                                        {c.country_code}
                                      </span>
                                    )}
                            ...
                            <td className="px-3 py-3 min-w-[180px]">
                              {c.isp || c.org || c.domain ? (
                                <div>
                                  ...
                                  {c.org && c.org !== c.isp && (
                                    <div
                                      className="text-slate-500 text-xs truncate max-w-[200px]"
                                      title={c.org}
                                    >
                                      {c.org}
                                    </div>
                            ...
                            <td className="px-3 py-3 whitespace-nowrap">
                              {isActive ? (
                                ...
                                <span className="text-slate-500">
                                  {fmtSessionDur(c.opened_at, c.closed_at)}
                                </span>
                              )}
                            </td>
                            ...
                            {/* User Agent */}
                            <td
                              className="px-3 py-3 text-slate-500 max-w-[200px] truncate"
                              title={c.user_agent ?? ""}
                            >
                              {c.user_agent ?? "—"}
                            </td>
...
function ChannelListItem({
  ...
  const rating =
    Number(channel.ratings_avg_rating) || Number(channel.average_rating) || 0;
  ...
  return (
    <div
      ...
      className={`
        ${
          focusProps.className
        } w-full text-left bg-slate-800 cursor-pointer relative
        ${
          isActive
            ? "ring-2 ring-primary scale-[1.02] z-10 shadow-xl"
    ...
    >
      ...
      <div className="w-full aspect-video relative bg-slate-900 text-slate-800">
          ...
          <>
            <div
              className={`absolute inset-0 bg-slate-800 animate-pulse ${
                isLoadingImage ? "opacity-100 z-10" : "opacity-0 -z-10"
              } transition-opacity`}
            />
            <img
              ...
              className={`w-full h-full object-contain p-2 opacity-90 group-hover:opacity-100 transition-opacity ${
                isLoadingImage ? "opacity-0" : "opacity-90"
              }`}
            />
          </>
        ) : (
        ...
        <button
          ...
          className={`absolute top-2 right-2 z-20 p-1.5 rounded-full backdrop-blur-sm transition-all hover:scale-110 ${
            liked
              ? "bg-white/10 text-red-500"
              : "bg-black/40 text-white opacity-100 lg:opacity-0 lg:group-hover:opacity-100"
          }`}
        >
        ...
        {/* Active Overlay */}
        <div
          className={`absolute inset-0 flex items-center justify-center bg-black/40 transition-opacity backdrop-blur-[2px] pointer-events-none ${
      ...
      <div className={`w-full p-2 mb-0.5`}>
        <h3
          className={`font-bold text-white truncate text-xs sm:text-sm leading-tight`}
        >
          {channel.name}
        </h3>
      ...
      <div
        className={`w-full flex md:flex-row items-center justify-between px-2 pb-2 gap-1`}
      >
...
function ClassicMenuButton({ onClick }: { onClick: () => void }) {
  const { focusProps, isFocused } = useTVFocus({
    onEnter: onClick,
    className:
      "p-2 rounded-lg transition-all duration-200 flex items-center gap-2 group",
    focusClassName:
    ...
    <button
      ...
      className={`${focusProps.className} ${
        isFocused
          ...
          : "bg-slate-900 border border-slate-700/50 hover:border-primary/50 text-slate-400 hover:text-white"
      }`}
    >
