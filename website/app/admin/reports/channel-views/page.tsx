...
import { Download, Calendar, ArrowLeft, ChevronDown, Image, Eye, X, Globe, Users, BarChart2 } from 'lucide-react';
...
interface DetailModal {
    open: boolean;
    loading: boolean;
    channel: ChannelViewStat | null;
    details: ChannelViewDetail[];
...
export default function ChannelViewsReportPage() {
    ...
    const [detailModal, setDetailModal] = useState<DetailModal>({
        open: false, loading: false, channel: null, details: [], summary: null,
    });

    useEffect(() => { fetchChannels(); }, []);
    useEffect(() => { fetchReport(); }, [dateRange, selectedChannel, status]);
    ...
    const fetchChannels = async () => {
        try {
            ...
            if (res.data?.data?.data) setChannels(res.data.data.data);
        } catch { /* ignore */ }
    };
    ...
    const searchChannelsAPI = async (query: string) => {
        ...
        setIsSearching(true);
        try {
            const res = await adminApi.get('/admin/channels', { params: { search: query } });
            if (res.data?.data?.data) {
                ...
                if (uniqueNew.length > 0) setChannels([...channels, ...uniqueNew]);
            }
    ...
    const openDetailModal = useCallback(async (channel: ChannelViewStat) => {
        setDetailModal({ open: true, loading: true, channel, details: [], summary: null });
        try {
            const res = await adminApi.get('/admin/reports/channel-view-details', {
                params: {
            ...
            const { details, summary } = res.data.data;
            setDetailModal(prev => ({ ...prev, loading: false, details, summary }));
        } catch {
    ...
    const closeDetailModal = () =>
        setDetailModal({ open: false, loading: false, channel: null, details: [], summary: null });
    ...
    const downloadJPEG = async () => {
        ...
        if (!el) return;
        try {
            const canvas = await html2canvas(el, { backgroundColor: '#0a0a0a', scale: 2, logging: false, useCORS: true });
            canvas.toBlob(blob => {
                if (!blob) return;
                const link = document.createElement('a');
    ...
    const handleDateChange = (type: 'start' | 'end', value: string) =>
        setDateRange(prev => ({ ...prev, [type === 'start' ? 'startDate' : 'endDate']: value }));
            ...
            <div id="report-content">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
                    ...
                    <div className="flex flex-wrap items-center gap-3 relative">
                        ...
                        <button onClick={downloadJPEG} className="flex items-center gap-2 px-4 py-2 bg-green-600 hover:bg-green-700 text-white rounded-lg transition-colors">
                            <Image size={18} /><span>Export JPEG</span>
                        </button>
                        <button onClick={downloadCSV} className="flex items-center gap-2 px-4 py-2 bg-primary hover:bg-primary/90 text-white rounded-lg transition-colors">
                                    ...
                                    tableData.map((channel, index) => (
                                        <tr key={index} className="hover:bg-white/5 transition-colors">
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap text-center">
                                                <button
                                                    onClick={() => openDetailModal(channel)}
                                                    title="View IP details"
                                                    className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-primary/10 hover:bg-primary/20 text-primary border border-primary/20 text-xs font-medium transition-colors"
                                                >
                                                    <Eye size={13} /> View
                                                </button>
                                            </td>
