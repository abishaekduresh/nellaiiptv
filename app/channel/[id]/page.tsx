...
export default function ChannelPage() {
  ...
  const checkAndIncrementView = async () => {
    if (playerRef.current && !playerRef.current.paused() && !hasIncrementedRef.current) {
      ...
      console.log('View not incremented. Paused:', playerRef.current?.paused(), 'Already incremented:', hasIncrementedRef.current);
    }
  };
  ...
  const decrementView = async () => {
    try {
      ...
      console.error('Failed to decrement view:', err);
    }
  };
  ...
  const handleRating = async (stars: number) => {
    ...
    try {
      ...
      console.error('Error rating channel:', err.response?.data || err);
